...
describe "update an animal route", :type => :request do
  ...
  before do
    put "/api/v2/animals/#{Animal.first.id}", params: { :species => 'Dog', :breed => 'Boxer', :age => 14, :name => 'Sadie', :gender => 'Female'}
  end
  ...
  it 'returns a 404 status not found' do
    ...
    put "/api/v2/animals/12", params: {name: @new_name, breed: @new_breed}
